...
export function loadPatterns() {
  const compiled = [];
  // BUILTINパターンのコンパイル成功数（Fail-Closedチェックで使用）
  let builtinCount = 0;
  // EXTRAパターンのスキップ数（デバッグ用）
  let extraSkipped = 0;

  // pubPathが存在する場合はneko-not-yoshiのNGワードを使用（BUILTIN代替）
  const pubPath = join(NEKO_NOT_YOSHI_DIR, 'ngwords.public.json');
  let usedPubPath = false;
  if (existsSync(pubPath)) {
    ...
    // pubPathがない場合のみBUILTIN_PATTERNSを直接使用し、成功数をカウントする
    for (const p of BUILTIN_PATTERNS) {
      try { compiled.push(compileBasePattern(p)); builtinCount++; } catch { /* skip */ }
    }
  }
  ...
  // EXTRAパターンのコンパイル（失敗時はスキップしてログ出力）
  for (const p of EXTRA_PATTERNS) {
    ...
    try {
      ...
      // EXTRAパターンのコンパイル失敗はスキップして記録する
      extraSkipped++;
      process.stderr.write(`[pii-mask-yoshi] EXTRA pattern skipped: ${p.id}\n`);
    }
  }

  // パターン配列とメタデータをまとめて返す（Fail-Closedチェックで使用）
  // usedPubPath: pubPath使用時はBUILTINカウントが0になるため、builtinチェックをスキップする判定に使う
  return { patterns: compiled, builtinCount, extraSkipped, usedPubPath };
}
