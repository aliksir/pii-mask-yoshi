...
const JP_SURNAMES_TOP50 = '長谷川|佐々木|佐藤|鈴木|高橋|田中|伊藤|渡辺|山本|中村|小林|加藤|吉田|山田|松本|井上|木村|斎藤|清水|山崎|阿部|池田|橋本|山口|石川|前田|小川|藤田|岡田|後藤|石井|村上|近藤|坂本|遠藤|青木|藤井|西村|福田|太田|三浦|藤原|岡本|松田|中川|中野|原田|小野|林|森';
...
const EXTRA_PATTERNS = [
  ...
  { id: 'jp-person-name-nospace', category: 'pii', regex: `(?:${JP_SURNAMES_TOP50})[\\u3400-\\u9FFF]{1,3}`, maskPrefix: 'PERSON' },
...
const NOSPACE_EXCLUDE = /[様氏殿社部課係長官局室員棟号館駅港橋寺院神塚山川池田畑]$/;
...
function validateJpNameNospace(matched) {
    ...
    try {
      compiled.push({
        ...
        validator: p.id === 'credit-card' ? (m) => luhnCheck(m) ? 'CARD' : null
          : p.id === 'jp-person-name' ? (m) => validateJpName(m)
          : p.id === 'jp-person-name-nospace' ? (m) => validateJpNameNospace(m) : null,
      });
    } catch { /* skip */ }
  }
