...
describe('maskText', () => {
  it('should mask Japanese phone numbers', () => {
    const result = maskText('電話番号は090-1234-5678です', 'test.txt');
  ...
  it('should mask email addresses', () => {
    const result = maskText('連絡先: tanaka@company.co.jp', 'test.txt');
  ...
  it('should mask global IPv4 addresses', () => {
    const result = maskText('サーバー: 203.0.113.50', 'test.txt');
  ...
  it('should mask private IPv4 with different prefix', () => {
    const result = maskText('LAN: 192.168.1.100', 'test.txt');
  ...
  it('should mask OpenAI API keys', () => {
    const result = maskText('OPENAI_API_KEY=sk-abcdefghijklmnopqrstuvwx', 'test.txt');
  ...
  it('should mask password key-value pairs', () => {
    const result = maskText('password=my_secret_123', 'test.txt');
  ...
  it('should mask Japanese password patterns', () => {
    const result = maskText('パスワード: secretpass', 'test.txt');
    assert.ok(result.includes('[PWA]'), `got: ${result}`);
    const findings = getStore().getFindings();
    assert.ok(findings.length > 0, 'should have findings');
    assert.equal(findings[0].confidence, 1.0);
  ...
  it('should mask credit card numbers with Luhn check', () => {
    const result = maskText('カード: 4111-1111-1111-1111', 'test.txt');
  ...
  it('should mask Japanese addresses', () => {
    const result = maskText('住所: 東京都渋谷区神南1丁目', 'test.txt');
  ...
  it('should assign same token to same value', () => {
    const result = maskText('TEL: 090-1234-5678, FAX: 090-1234-5678', 'test.txt');
  ...
  it('should assign different tokens to different values', () => {
    const result = maskText('A: 090-1111-2222, B: 080-3333-4444', 'test.txt');
    assert.ok(result.includes('[電話A]'), `got: ${result}`);
  ...
  it('should mask home paths', () => {
    const result = maskText('file at C:\\Users\\tanaka\\Documents\\secret.txt', 'test.txt');
  ...
  it('should mask JWT tokens', () => {
    const result = maskText('Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.signature', 'test.txt');
  ...
  it('should mask AWS Secret Access Keys', () => {
    const result = maskText('Secret: wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY', 'test.txt');
  ...
  it('should mask Azure AccountKey in connection strings', () => {
    const result = maskText('DefaultEndpointsProtocol=https;AccountKey=abc123def456ghi789jkl012mno345==;', 'test.txt');
  ...
  it('should mask password with slash separator', () => {
    const result = maskText('管理者: admin / P@ssw0rd2026!', 'test.txt');
  ...
  it('should mask bank account with type prefix', () => {
    const result = maskText('三菱UFJ銀行 渋谷支店 普通 1234567', 'test.txt');
  ...
  it('should mask Japanese passport numbers', () => {
    const result = maskText('パスポート番号: TK1234567', 'test.txt');
  ...
  it('should mask corporate numbers', () => {
    const result = maskText('法人番号: 1234567890123', 'test.txt');
  ...
  it('should mask Japanese person names (surname + space + given name)', () => {
    const result = maskText('管理者: 山田 花子', 'test.txt');
  ...
  it('should mask Japanese person names with full-width space', () => {
    const result = maskText('担当: 佐藤　健太', 'test.txt');
    assert.ok(result.includes('[人名'), `got: ${result}`);
  ...
  it('should mask comma-separated name lists', () => {
    const result = maskText('共同編集者: 鈴木、田中、高橋、渡辺', 'test.txt');
    assert.ok(result.includes('[人名'), `got: ${result}`);
  ...
  it('should mask CJK Extension A characters in names', () => {
    const result = maskText('担当: 木村 次郎', 'test.txt');
    assert.ok(result.includes('[人名'), `got: ${result}`);
    const findings = getStore().getFindings();
    assert.ok(findings.length > 0, 'should have findings');
    assert.equal(findings[0].confidence, 0.9);
  ...
  it('should mask person name with honorific suffix (さん)', () => {
    getStore().clear();
    const result = maskText('渡辺さんに連絡してください', 'test.txt');
    assert.ok(result.includes('[人名'), `got: ${result}`);
  ...
  it('should mask person name with title suffix (部長)', () => {
    getStore().clear();
    const result = maskText('松本部長が承認しました', 'test.txt');
    assert.ok(result.includes('[人名'), `got: ${result}`);
  ...
  it('should mask nospace name followed by particle', () => {
    getStore().clear();
    const result = maskText('佐藤太郎が発表した', 'test.txt');
    assert.ok(result.includes('[人名'), `got: ${result}`);
    const findings = getStore().getFindings();
    assert.ok(findings.length > 0, 'should have findings');
  ...
  it('should mask name after label (Top50外の姓)', () => {
    getStore().clear();
    const result = maskText('氏名：木下美咲', 'test.txt');
    assert.ok(result.includes('[人名'), `got: ${result}`);
  ...
  it('should mask Top200 surname without space (藤井)', () => {
    getStore().clear();
    const result = maskText('藤井太郎が参加した', 'test.txt');
    assert.ok(result.includes('[人名'), `got: ${result}`);
    const findings = getStore().getFindings();
    assert.ok(findings.length > 0, 'should have findings');
    assert.equal(findings[0].confidence, 0.7);
  ...
  it('should mask residence card number', () => {
    getStore().clear();
    const result = maskText('在留カード: AB12345678CD', 'test.txt');
  ...
  it('should mask pension number with context', () => {
    getStore().clear();
    const result = maskText('基礎年金番号: 1234-567890', 'test.txt');
  ...
  it('should mask driver license number with context', () => {
    getStore().clear();
    const result = maskText('運転免許証番号: 123456789012', 'test.txt');
  ...
  it('should mask URLs', () => {
    getStore().clear();
    const result = maskText('詳細は https://example.com/path?q=1 を参照', 'test.txt');
  ...
  it('should mask Ethereum address', () => {
    getStore().clear();
    const result = maskText('ETH: 0x742d35Cc6634C0532925a3b844Bc9e7595f2bD68', 'test.txt');
  ...
  it('should mask valid IBAN', () => {
    getStore().clear();
    const result = maskText('IBAN: GB29NWBK60161331926819', 'test.txt');
  ...
  it('should mask jumin code with context', () => {
    getStore().clear();
    const result = maskText('住民票コード: 12345678901', 'test.txt');
...
describe('min_confidence threshold (T3)', () => {
  it('should mask all by default (no options)', () => {
    getStore().clear();
    const result = maskText('tanaka@example.com 鈴木、田中、高橋、渡辺');
    assert.ok(result.includes('[メール'), `email should be masked: ${result}`);
    assert.ok(result.includes('[人名'), `name list should be masked: ${result}`);
  });

  it('should mask all with min_confidence=0.0', () => {
    getStore().clear();
    const result = maskText('tanaka@example.com 鈴木、田中、高橋、渡辺', null, { min_confidence: 0.0 });
    assert.ok(result.includes('[メール'), `email should be masked: ${result}`);
    assert.ok(result.includes('[人名'), `name list should be masked: ${result}`);
  });

  it('should filter low confidence with min_confidence=0.8', () => {
    getStore().clear();
    const result = maskText('tanaka@example.com 鈴木、田中、高橋、渡辺', null, { min_confidence: 0.8 });
    assert.ok(result.includes('[メール'), `email(1.0) should be masked: ${result}`);
    assert.ok(!result.includes('[人名'), `name-list(0.5) should NOT be masked: ${result}`);
  });

  it('should only mask perfect confidence with min_confidence=1.0', () => {
    getStore().clear();
    const result = maskText('tanaka@example.com 山田 花子', null, { min_confidence: 1.0 });
    assert.ok(result.includes('[メール'), `email(1.0) should be masked: ${result}`);
    assert.ok(!result.includes('[人名'), `person-name(0.9) should NOT be masked: ${result}`);
  });

  it('should include boundary value (>=) with min_confidence=0.9', () => {
    getStore().clear();
    const result = maskText('山田 花子が参加した', null, { min_confidence: 0.9 });
    assert.ok(result.includes('[人名'), `person-name(0.9) should be masked with threshold 0.9: ${result}`);
  });
});

describe('instSuffix - 地 suffix (T1)', () => {
  it('should not mask 検地 as person name list', () => {
    getStore().clear();
    const result = maskText('検地、封地、知行地');
    assert.ok(!result.includes('[人名'), `should not mask institutional suffixes: ${result}`);
  });
});
