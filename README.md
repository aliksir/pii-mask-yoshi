...
- **Pattern obfuscation**: Pattern definitions are XOR-obfuscated at build time. No plaintext patterns appear in the source or the published package.
...
Pattern definitions in `src/encoded-data.mjs` are XOR-obfuscated (not encrypted). This prevents casual plaintext grep but is not a cryptographic protection. See [THREAT-MODEL.md](THREAT-MODEL.md) for details.

For the key management roadmap (XOR → AES-GCM → KMS integration), see [docs/KEY-MANAGEMENT.md](docs/KEY-MANAGEMENT.md).
